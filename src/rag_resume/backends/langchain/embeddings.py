...
from rag_resume.json import JsonCodecProtocol, enforce_dict_type
...
class LangchainVectorStore(ABC, VectorStoreProtocol[MetadataType]):
    ...
    def _filter_adapter(
        ...
        return filter_func(
            Document(
                ...
                metadata=self.metadata_codec.convert_json(document.metadata),
            )
        )

    def add(self, text: Sequence[str]) -> Sequence[uuid.UUID]:
        return [uuid.UUID(uuid_str) for uuid_str in self.vector_store.add_texts(text)]

    def add_with_metadata(self, text: Sequence[str], metadata: list[MetadataType]) -> Sequence[uuid.UUID]:
        return [
            uuid.UUID(uuid_str)
            for uuid_str in self.vector_store.add_texts(
                text,
                metadatas=[
                    enforce_dict_type(self.metadata_codec.encode_python_json(metadatum)) for metadatum in metadata
                ],
            )
        ]
    ...
    def lookup(
        ...
        return [
            Document(
                id=uuid.UUID(doc.id), content=doc.page_content, metadata=self.metadata_codec.convert_json(doc.metadata)
            )
